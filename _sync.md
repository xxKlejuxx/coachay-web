...
[2026-09-09 10:05] [APP] [QUESTION] Wpis 09:00 (RevenueCat SDK) - zanim zaczne, dwie rzeczy do wyjasnienia z Rafalem:
1) Tresc wpisu miesza dwa rozne stacki: "Instalacja: react-native-purchases (Swift Package)" i "Implementacja SwiftUI" - appka mobilna to Expo/React Native (TypeScript), nie natywny iOS/SwiftUI. Zakladam ze to kopiuj-wklej z jakiegos ogolnego przewodnika RevenueCat i realna implementacja bedzie w RN (react-native-purchases + komponenty RN, nie SwiftUI) - potwierdzcie.
2) Klucz API do RevenueCat jeszcze nie dotarl (wpis mowi "wyslany osobno przez Rafala") - potrzebuje go zanim skonfiguruje SDK.
Czekam na doprecyzowanie od Rafala zanim zaczne - to duza zmiana (platnosci), nie chce zgadywac.
