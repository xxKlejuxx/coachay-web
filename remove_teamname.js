...
async function removeBadTeamName() {
    try {
        console.log('=== Usuwanie popsutych teamName ===');
        console.log('');
        ...
        // 2. Usuñ teamName z dru¿yn w klubie
        console.log('2. Usuwanie teamName z dru¿yn...');
        ...
        for (let i = 0; i < clubTeams.length; i++) {
            ...
            // Sprawd¼ czy mam popsute teamName
            const hasTeamName = fields.teamName?.stringValue;
            
            if (!hasTeamName) {
                console.log(`${i + 1}. ${teamId} - brak teamName do usuniêcia`);
                continue;
            }
            
            // Usuñ pole teamName
            const updateResponse = await httpsPatch(`${baseUrl}/teams/${teamId}?updateMask.fieldPaths=teamName`, {
                fields: {}
            });
            
            if (updateResponse.statusCode === 200) {
                console.log(`${i + 1}. ${teamId} - usuniêto teamName`);
            } else {
        ...
        console.log('');
        console.log('=== Zakoñczono usuwanie teamName ===');
        
    } catch (error) {
        console.error('B³ad podczas usuwania teamName:', error.message);
    }
}
