...
async function checkTrainerViaREST() {
    try {
        ...
        console.log('Request URL:', trainerUrl);
        ...
        console.log('Status code:', trainerResponse.statusCode);
        console.log('Response headers:', trainerResponse.headers);
        console.log('Response body:', trainerResponse.body);
        
        if (trainerResponse.statusCode !== 200) {
            console.log('❌ Błąd HTTP:', trainerResponse.statusCode);
            return;
        }
        
        try {
            const trainerData = JSON.parse(trainerResponse.body);
            ...
            if (trainerData.documents && trainerData.documents.length > 0) {
                ...
                console.log('Membership status code:', membershipResponse.statusCode);
                console.log('Membership response body:', membershipResponse.body);
                
                if (membershipResponse.statusCode !== 200) {
                    console.log('❌ Błąd HTTP memberships:', membershipResponse.statusCode);
                    return;
                }
                
                const membershipData = JSON.parse(membershipResponse.body);
            ...
            console.error('Błąd podczas parsowania JSON:', e.message);
        }
