...
async function addNamesToTeams() {
    try {
        console.log('=== Dodawanie nazw do dru¿yn ===');
        console.log('');
        ...
        // 2. Dodaj nazwy do dru¿yn w klubie
        console.log('2. Dodawanie nazw do dru¿yn...');
        ...
        for (let i = 0; i < clubTeams.length; i++) {
            ...
            // Sprawd¼ czy nazwa ju¿ istnieje
            if (fields.name?.stringValue) {
                console.log(`${i + 1}. ${teamId} - nazwa ju¿ istnieje: ${fields.name.stringValue}`);
                continue;
            }
            ...
            const updateData = {
                fields: {
                    ...
                    displayName: { stringValue: teamName }
                }
            };
            
            const updateResponse = await httpsPatch(`${baseUrl}/teams/${teamId}?updateMask.fieldPaths=name&updateMask.fieldPaths=displayName`, updateData);
            
            if (updateResponse.statusCode === 200) {
                console.log(`${i + 1}. ${teamId} - dodano nazwê: ${teamName}`);
            } else {
        ...
        console.log('');
        console.log('=== Zakoñczono dodawanie nazw ===');
        
    } catch (error) {
        console.error('B³ad podczas dodawania nazw:', error.message);
    }
}
